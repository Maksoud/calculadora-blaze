...
function jogadaAutomatizada(ultimaCor) {
	...
	if (jogandoAut == 4) jogandoAut = false
	...
	if (ganhouCor == 4) {
		...
		console.log("GANHOU!")
		console.log("AGUARDANDO PRÓXIMO SINAL!")
	...
	if (jogandoAut) {
		...
		if (cVermelho <= 2) {
			...
			console.log("ENTRA NO VERMELHO AUT")

			if (ultimaCor == "red") ganhouCor++
		...
		if (cVermelho > 2) {
			...
			console.log("ENTRA NO PRETO AUT")
			
			cPreto += 1
			if (ultimaCor == "black") ganhouCor++
...
function analisarJogos() {
	...
	try {
		...
		if (ultimoSinal.firstChild.classList[1] == "red") {
			...
			// Continuar jogando
			if (jogando) jogar()
			if (jogandoAut) jogadaAutomatizada("red")
			...
			// Continuar jogando
			if (jogando) jogar()
			if (jogandoAut) jogadaAutomatizada("black")
